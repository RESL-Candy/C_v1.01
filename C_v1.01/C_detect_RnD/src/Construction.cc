...
G4VPhysicalVolume* MyDetectorConstruction::Construct()
{
  ...
  //G4MaterialPropertiesTable* LS_Surface = new G4MaterialPropertiesTable();
  //LS_Surface->AddProperty("REFLECTIVITY", ephoton, reflectivity);
  //LS_Surface->AddProperty("EFFICIENCY", ephoton, efficiency);
  
  G4OpticalSurface* Tef_roughSurf = new G4OpticalSurface("tef_roughSurf");
  Tef_roughSurf->SetType(dielectric_LUT);
  Tef_roughSurf->SetModel(LUT);
  Tef_roughSurf->SetFinish(polishedteflonair);//이거 다른 세팅이면 반사가 안됨. ㅠ
  
  Tef_roughSurf->SetSigmaAlpha(1.0);//0~1 : grounded ~ polished
  Tef_roughSurf->SetMaterialPropertiesTable(Teflon_Surface);
  
  //G4OpticalSurface* LAB_roughSurf = new G4OpticalSurface("LAB_roughSurf");
  //LAB_roughSurf->SetType(dielectric_LUT);
  //LAB_roughSurf->SetModel(LUT);
  //LAB_roughSurf->SetFinish(polishedteflonair);//이거 다른 세팅이면 반사가 안됨. ㅠ
 
  //LAB_roughSurf->SetSigmaAlpha(1.0);//0~1 : grounded ~ polished
  //LAB_roughSurf->SetMaterialPropertiesTable(LS_Surface);
  ...
  fmat_Prop_table_LAB->AddConstProperty("SCINTILLATIONYIELD", 1. / MeV);
  ...
  G4LogicalVolume* teflonLog = new G4LogicalVolume(teflonSol,
						   Mat_Teflon,
						   "Teflonlog");
  G4LogicalSkinSurface* Tef_skinSurface = new G4LogicalSkinSurface("Teflon_Surface",teflonLog,Tef_roughSurf);
  ...
  G4LogicalVolume* LS_Log = new G4LogicalVolume(LS_Sol,
						fMat_LAB,
						"LAB_Log");
  //G4LogicalSkinSurface* LAB_skinSurface = new G4LogicalSkinSurface("LS_Surface",LS_Log,LAB_roughSurf);
  ...
  G4Box* SiPM_sol = new G4Box("SiPM_sol",.5*SD_width,SD_height,.5*SD_width);
